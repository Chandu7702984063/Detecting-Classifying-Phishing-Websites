...
#training the models
    ...
    #1. RANDOM FOREST
from sklearn.ensemble import RandomForestClassifier#importing function from sklearn library
rf = RandomForestClassifier(n_estimators=100,max_features='sqrt')#calling the algorithm
rf.fit(X_train,y_train)#training the model
y_pred_rf = rf.predict(X_test)#testing the model
print(classification_report(y_test,y_pred_rf,target_names=['safe','not safe']))

joblib.dump(rf, 'rf_model.pkl')#saving the model as pickle file to be called later, in order to reduce computation time

    #2. LIGHT GBM CLASSIFIER
from lightgbm import LGBMClassifier#importing function from sklearn library
lgb = LGBMClassifier()#calling the algorithm
LGB_C = lgb.fit(X_train, y_train)#training the model
y_pred_lgb = LGB_C.predict(X_test)
print(classification_report(y_test,y_pred_lgb,target_names=['safe','not safe']))#testing the model

joblib.dump(lgb, 'lgb_model.pkl')#saving the model as pickle file to be called later, in order to reduce computation time
